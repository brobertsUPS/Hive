...
import { getTopNode, isSurrounded, possibleMoves, addNode } from '../utility/BoardFunctions';
  ...
  setBoard(newBoard) {
	...
	*/
	nextBoard({ move, AIMove }) {
		...
		this.setState((prevState) => {
			return {
				curPlayer: prevState.curPlayer === Colors.WHITE ? Colors.BLACK : Colors.WHITE, // swap players
				turn: prevState.turn === Colors.BLACK ? prevState.turn + 1 : prevState.turn // increment turn if tiles are BLACK
			};
		});
		this.setState(prevState => {
			return {
				tempMoveState: {
					node: new Node({ type: prevState.tempMoveState.node.type, color: prevState.curPlayer })
				}
			};
		});
	}
	...
	*/
	...
	_renderHex({ node = {}, x, y }) {
		...
		return (
			<div className={`hex`} onClick={makeUserSpecifiedMove}>
				<div className={`top ${node.color}`}></div>
				<div className={`middle ${node.color}`}>{node.type || 'null'} {node.color || 'null'} {x} {y}</div>
				<div className={`bottom ${node.color}`}></div>
			</div>
		);
	}
	...
	_renderHexRow({ even, rowItems }) {
		return (
			<div className={`hex-row ${even ? 'even' : ''}`}>
				{rowItems}
			</div>
		);
	}
	...
	render() {
		...
		return (
			<div>
				type: <input type="text" value={this.state.tempMoveState.node.type} onChange={this.changeNodeType} />
				color: {this.state.tempMoveState.node.color}

				<br />
				<div>minX:{minX} maxX:{maxX}</div>
				<div>minY:{minY} maxY:{maxY}</div>
				<br />
